...
import { PASSPORT_ADDRESS } from '@/lib/contracts';
  ...
  const handleClaim = async () => {
    ...
    try {
      const contract = new Contract(
        [{ name: 'transfer_from', type: 'function', inputs: [
      ...
      const tx = await contract.transfer_from(
        account.address,
        account.address,
        { low: params.tokenId, high: '0' }
      );
      ...
      <div style={{ padding: '24px 0', borderTop: '1px solid #333', borderBottom: '1px solid #333' }}>
        ...
        {account && status === 'idle' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <button 
              onClick={() => disconnect()} 
              style={{ marginTop: '16px', background: 'none', border: 'none', color: '#666', cursor: 'pointer', fontSize: '14px' }}
            >
              Disconnect
            </button>
          </div>
        ...
        {status === 'claiming' && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <p style={{ color: '#D4AF37' }}>Confirming on Starknet...</p>
            <p style={{ color: '#666', fontSize: '12px', marginTop: '8px' }}>Please confirm in your wallet</p>
          </div>
        )}
